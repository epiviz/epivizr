...
Once the browser is open and is connected to an active epivizr session, the epivizr session registers available chart types supported in the browser client. To list registered chart types

```{r, eval=TRUE}
app$chart_mgr$list_all_available_chart_types()
```
...
### Modify default chart settings and colors

To modify default chart colors or settings, we need to know what settings can be applied to the chart.

There are two ways to set settings and colors to a chart. 

* When plotting the chart

If using the plot function, use the `list_all_available_chart_types` method to list settings that can be applied to a chart type. For example, for a blocksTrack chart, we can set `minBlockDistance` as a setting for the plot function.
...
* if the block is already added to the session, use the `set_chart_settings` method to update settings and colors. Since the block track is already added to the browser, you can also use `get_chart_settings` to list the settings that are currently applied to the plot.

```{r, eval=TRUE}
# to list applied chart settings
app$chart_mgr$get_chart_settings(blocks_chart)
...
plots and visualization can be saved through the epivizr session as a pdf or png. To do so, use the `print_chart` method.
...
We have a number of available data types in `epivizr`. In the previous section, we used the `block` type. For the methylation data at base-pair resolution in the `tcga_colon_curves` object, we will use the `bp` type.
...
To print your workspace as a pdf or png, we use the `print_workspace` method
