...
app <- startEpiviz(debug=TRUE, openBrowser=FALSE, nonInteractive=TRUE, try_ports=TRUE)
...
```{r,eval=TRUE}
blocks_chart <- app$plot(colon_blocks, datasource_name="450k colon_blocks")
```
...
app$data_mgr$update(ms_obj, colon_blocks[keep,])
```
...
means_track <- app$plot(colon_curves, datasource_name="450kMeth",type="bp",columns=c("cancerMean","normalMean"))
```
...
```{r, eval=TRUE}
diff_chart <- app$plot(colon_curves,"450kMethDiff",type="bp",columns=c("smooth"),ylim=matrix(c(-.5,.5),nc=1))
```
...
We can use the session manager to list charts we have added so far, or to remove charts.

```{r, eval=TRUE}
app$chart_mgr$list_charts()
app$chart_mgr$rm_chart(means_chart)
app$chart_mgr$list_charts()
```
...
se_chart <- mgr$plot(sumexp, datasource_name="Mean by Sample Type", columns=c("normal", "cancer"))
```
...
To close the connection to `Epiviz` and remove all tracks added during the interactive session, we use the `stop_server` function.
