...
This changes the color map for the second blocks track added.
To list can also use `print_chart_info` to list settings and colors currently used in the chart.
...
There are a number of different data types available to use through `epivizr`. In the previous section, we used the `block` type. To visualize methylation data at base-pair resolution from the `tcga_colon_curves` object, we will use the `bp` type.
